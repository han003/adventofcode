...
(function () {
    ...
    const OPERATIONAL = '.';
    const DAMAGED = '#';
    function getConfig(line, unfold) {
        const [springData, configData] = line.split(' ');
        return {
            springData: Array.from({ length: unfold ? 5 : 1 }, () => 0).reduce((acc, _, i) => acc.concat(springData), []).join(unfold ? '?' : ''),
            config: Array.from({ length: unfold ? 5 : 1 }, () => configData.split(',').map((x) => parseInt(x))).flat(),
        };
    }
    function getAllPossibleLines(config) {
        let possibilities = 0;
    ...
    function lineMatchesConfig(line, config) {
